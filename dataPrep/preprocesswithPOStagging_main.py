...
for provname in provnames:
    ...
    newptlist = []
    for doc in ptlist:
        temp = chunker.cutwithPOS(doc)
        temp1 = chunker.removeProvs(temp)   # remove provs and cities
        newptlist.append('\n'.join(temp1))
        count += 1
    ...
    raw_df.to_csv(r'D:\3policyAyc\_database\_policytxt\Wordlist_'+provname+'5.csv', encoding="utf_8_sig", index=False)
...
for prov in provnames:
    tempdf = pd.read_csv(r'D:\3policyAyc\_database\_policytxt\Wordlist_'+prov+'5.csv')
    dfls0.append(tempdf)
# dfls0[1].drop(index=)
dfall0 = pd.concat(dfls0, ignore_index=True)
dfall0.to_csv(r'D:\3policyAyc\_database\_policytxt\Wordlist_all5.csv', encoding="utf_8_sig", index=False)
# #
# count = 0
# for tdf in dfls0:
#     print(tdf.shape[0])
#     count += tdf.shape[0]
# print('sum:'+str(count))
# print(dfall0.shape[0])
#
#
# df = pd.read_csv(r'D:\3policyAyc\_database\_policytxt\Wordlist_四川3.csv')
# for i, j in enumerate(df['title'].to_list()):
#     if '省商务厅关于组织企业参加第九届中国—东盟博览会通知' in j:
#         print(df['ptext'].to_list()[i])
